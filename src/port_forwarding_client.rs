use libp2p::{
    dcutr, identify, multiaddr::Protocol, noise, relay, swarm::NetworkBehaviour, tcp, yamux,
    Multiaddr, PeerId,
};
...
impl PortForwardingClient {
    ...
    pub async fn stop(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(handle) = self.join_handle.take() {
            handle
                .join()
                .map_err(|e| anyhow::anyhow!("Failed to join client task: {:?}", e))?;
        }
        Ok(())
    }
    ...
    pub fn start(&mut self) -> anyhow::Result<()> {
        ...
        let join_handle = std::thread::spawn(move || {
            ...
            runtime.block_on(async move {
                if let Err(e) = PortForwardingClient::start_internal(
                    server_id,
                    local_forward_addr,
                    config,
                    stop_receiver,
